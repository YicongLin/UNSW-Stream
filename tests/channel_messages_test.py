...
from src.channel import channel_join_v2, channel_messages_v2, \
                        channel_details_v2, channel_invite_v2
...
def valid():
    ...
    channel_id_1 = channels_create_v2(token_1, "qwe", True)['channel_id']
    channel_id_2 = channels_create_v2(token_2, "asd", False)['channel_id']
    start_0 = 0
    return token_1, token_2, channel_id_1, channel_id_2, start_0

# Testing for an invalid channel
def test_invalid_channel(valid):
    token_1, _, _, _, start_0 = valid
    with pytest.raises(InputError):
        channel_messages_v2(token_1, "invalid_channel", start_0)
        
# Testing for a valid channel
def test_valid_channel(valid):
    token_1, _, channel_id_1, _, start_0 = valid
    channel_messages_v2(token_1, channel_id_1, start_0)
...
def test_start_greater(valid):
    token_1, _, channel_id_1, _, start_0 = valid
    with pytest.raises(InputError):
        channel_messages_v2(token_1, channel_id_1, 2)
...
def test_not_a_member(valid):
    token_1, _, _, channel_id_2, start_0 = valid
    with pytest.raises(AccessError):
        channel_messages_v2(token_1, channel_id_2, start_0)
...
def test_empty():
    with pytest.raises(InputError):
        channel_messages_v2("", "", 0)

def test_invalid_strings():
    with pytest.raises(InputError):         
        channel_messages_v2("invalid_id_1", "invalid_channel", 1)

def test_symbols():
    with pytest.raises(InputError):
        channel_messages_v2("#&$_*%", "#$(%}(", 1)

def test_combination():
    with pytest.raises(InputError):
        channel_messages_v2("", "", 1)
        channel_messages_v2("", "#$(%}(", 1)
        channel_messages_v2("invalid_id_1", "", 1)
        channel_messages_v2("invalid_id_1", "invalid_channel", 1)
        channel_messages_v2("#&$_*%", "", 1)
        channel_messages_v2("!@#$%^", "invalid_channel", 1)
