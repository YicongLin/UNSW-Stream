...
def test_dm_create():
    ...
    u_id = response['u_id']
    login = requests.post(f'{BASE_URL}/auth/login/v1', json={"email": "test@gmail.com", "password": "password454643"})
    ...
    value = requests.get(f'{BASE_URL}/dm/list/v1', json={"token": token})
    assert value.json() == {
        'dms': [
            {
                'dm_id': dm_id,
...
def test_invalid_id():
    ...
    login = response = requests.post(f'{BASE_URL}/auth/login/v2', json={"email": "test40@gmail.com", "password": "password454643"})
    token = json.loads(login.text)['token']

    requests.post(f'{BASE_URL}/dm/create/v1', params={"token": token, "u_ids": [-9]})
    assert (response.status_code) == 400

    requests.post(f'{BASE_URL}/dm/create/v1', params={"token": token, "u_ids": [-9, 10, 11]})
    assert (response.status_code) == 400

    requests.post(f'{BASE_URL}/dm/create/v1', json={"token": token, "u_ids": [u_id, -1]})
    assert (response.status_code) == 400 """