...
def test_invalid_token(clear_setup, register_second, channel_two):
...
# Testing valid case
def test_valid(clear_setup, register_first, channel_one, channel_two):
    # first user registers; obtain token
    token = register_first['token']
    ...
    # second user creates channel; obtain channel_id
    channel_id = channel_two['channel_id']
    # first user joins the second channel
    payload1 = {
        "token": token,
        "channel_id": channel_id
    }
    requests.post(f'{BASE_URL}/channel/join/v2', json = payload1)
    # first user leaves channel
    payload2 = {
        "token": token,
        "channel_id": channel_id
    }
...
def test_user_left(clear_setup, register_first, register_second, channel_two):
    ...
    response = r.json()
    assert response == {"name": "2", "is_public": True, "owner_members": [], "all_members": [member]}
