...
def test_invalid_message_id(setup_clear, registered_second):
    ...
    token = registered_second['token']
    # second user attempts to react to a message with an invalid message_id
    payload = {
        "token": token,
        "message_id": 7,
        "react_id": 1
...
def test_invalid_react_id(setup_clear, registered_second, channel_two):
    ...
    # second user attempts to react to a message with an invalid react_id
    payload = {
        "token": token,
        "message_id": 1,
        "react_id": 2
...
# Testing for a case where the message in a channel already contains a react from the authorised user
def test_already_reacted_channel(setup_clear, registered_first, channel_one):
    ...
    # first user reacts to the message
    payload2 = {
        "token": token,
        "message_id": 1,
        "react_id": 1
    ...
    # first user attempts to react to the message again
    payload2 = {
        "token": token,
        "message_id": 1,
        "react_id": 1
...
# Testing for a case where the message in a DM already contains a react from the authorised user
...
# Testing for a successful react in a channel
def test_successful_react_channel(setup_clear, registered_first, registered_second, channel_one, channel_two):
    # first user registers; obtain token
    ...
    r = requests.post(f'{BASE_URL}/message/react/v1', json = payload2)
    assert r.status_code == 200
    # second user requests channel messages; the list of u_ids should contain u_id_2,
    # 'is_this_user_reacted' should be 'True'
    payload3 = {
        "token": token_2,
        "channel_id": channel_id,
        "start": 0
    ...
    # first user requests channel messages; the list of u_ids should contain u_id_2,
...
def test_successful_react_dm(setup_clear, registered_first, registered_second, dm_one, dm_two):
    ...
    r = requests.post(f'{BASE_URL}/message/react/v1', json = payload2)
    assert r.status_code == 200
    # second user requests DM messages; the list of u_ids should contain u_id_2,
    # 'is_this_user_reacted' should be 'True'
    payload3 = {
        "token": token_2,
        "dm_id": dm_id,
    ...
    response = r.json()
    assert response == {"messages": [message], "start": 0, "end": -1}
    # first user requests DM messages; the list of u_ids should contain u_id_2,
    # 'is_this_user_reacted' should be 'False'
    payload4 = {
        "token": token_1,
        "dm_id": dm_id,
