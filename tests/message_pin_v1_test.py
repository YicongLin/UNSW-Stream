...
def test_invalid_token(setup_clear, registered_first):
    ...
    # first user attempts to pin a message
    payload = {
        "token": token,
        "message_id": 7
    }
...
def test_message_already_pinned_channel(setup_clear, registered_first, channel_one):
    # first user registers; obtain token
    token = registered_first['token']
    ...
    # first user pins the message again
    payload2 = {
        "token": token,
        "message_id": 1
    }
...
# Testing for when the message is already pinned in the DM
def test_message_already_pinned_dm(setup_clear, registered_first, dm_one):
    # first user registers; obtain token
    token = registered_first['token']
...
def test_no_owner_permissions_channel(setup_clear, registered_first, registered_second, channel_one):
    ...
    # second user attempts to pin the message
    payload3 = {
        'token': token_2,
        'message_id': 1
    }
...
# Testing for a successful pin in a channel
def test_successful_pin_channel(setup_clear, registered_first, channel_one):
    ...
    # first user requests channel messages; the 'is_pinned' key should be True
    payload3 = {
        "token": token,
        "channel_id": channel_id,
        "start": 0
...
def test_successful_pin_dm(setup_clear, registered_first, dm_one):
    # first user registers; obtain token and u_id
    token = registered_first['token']
    u_id = registered_first['auth_user_id']
    # first user creates DM with second user; obtain dm_id
